...
(function (domGlobals) {
    ...
    var setFloat = function (image, value) {
      image.style.float = value;
    };
    var getFloat = function (image) {
      return getStyle(image, 'float');
    };
    ...
    var getStyleValue = function (normalizeCss, data) {
      ...
      if (getFloat(image) || data.float !== '') {
        setFloat(image, data.float);
      }
    ...
    var write = function (normalizeCss, newData, image) {
      ...
      updateProp(image, oldData, newData, 'float', normalized(setFloat, normalizeCss));
    ...
    var formFillFromMeta2 = function (info, data, meta) {
      ...
      if (info.hasAdvTab) {
        ...
        if (isString(meta.float)) {
          data.float = meta.float;
        }
      }
    };
    ...
    var calcFloat = function (css) {
      return css['float'] ? String(css['float']) : '';
    };
    ...
    var changeHandler = function (helpers, info, state) {
      return function (api, evt) {
        ...
        } else if (evt.name === 'vspace' || evt.name === 'hspace' || evt.name === 'border' || evt.name === 'borderstyle' || evt.name == 'float') {
